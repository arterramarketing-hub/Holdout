...
function buildEnvironment() {
  ...
  const stones = [], balconies = [], spires = [], placed = [], detail = [], sleepers = [], tanks = [], graves = [], hedges = [], mounds = [];
  ...
  for (const [kind, fx, fz, rot] of TOWN.fixed) {
    ...
    if (kind === 'platform') { pushSlab(CN, fx, 0, fz, hw * 2, 0.9, hd * 2, colorOf('#bfbdb6'), 3.2); box(fx, 0.91, fz + hd - 0.12, hw * 2, 0.02, 0.18, '#e8c42a'); }
      ...
      graves.push([fx, 0.45, fz, 0.8, 0.9, 0.22, 0, '#ffffff']);
      if (hash2(fz, fx) < 0.5) { box(fx, 1.08, fz, 0.1, 0.5, 0.08, '#8a8a84'); box(fx, 1.18, fz, 0.36, 0.08, 0.08, '#8a8a84'); }
  ...
  staticInstances(new THREE.BoxGeometry(1, 1, 1), stoneMat, graves);
